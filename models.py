...
class Dynamic(nn.Module):
    def __init__(self, input_size, seq_len, hidden_size, output_size, nlayers, model='linear',
                 perron_lambda_min=0.1, perron_lambda_max=0.999):
        """
        Args:
            ...
            perron_lambda_min/max: eigenvalue range for Perron state matrix (only when model=='perron')
        """
        super().__init__()
        ...
        # The state matrix A
        if model == 'perron':
            self.fx = Linear.PerronFrobeniusLinear(
                hidden_size, hidden_size, bias=False,
                lambda_min=perron_lambda_min, lambda_max=perron_lambda_max
            )
        else:
    ...
    def forward(self, x0, u):
        ...
        # 兼容 pfNDM (有约束) 和 NDM (无约束线性层)
        if hasattr(self.fx, 'effective_W'):
            state_matrix = self.fx.effective_W()
        else:
            state_matrix = self.fx.weight
