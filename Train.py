...
def Train(arguments):
    ...
    if arguments.predict:
        ...
        # 可视化
        print("Drawing prediction plots...")
        pred_np = (100 * test_pred[:, 0, :]).cpu().numpy()
        true_np = (100 * test_true[:, 0, :]).cpu().numpy()
        ...
        fig.suptitle(f'Temperature Estimation vs Ground Truth ({arguments.model.upper()} Model)', fontsize=18,
                     fontweight='bold')

        for i in range(arguments.features):
            row = i // 2
            col = i % 2
            ax = axs[row, col]
            ax.plot(true_np[:, i], label='Measurement', color='steelblue', linewidth=2.5)
            ax.plot(pred_np[:, i], label='Estimation', color='indianred', linestyle='--', linewidth=2.5)
        ...
        save_path = f'./checkpoints/{arguments.model}_prediction_plot.png'
